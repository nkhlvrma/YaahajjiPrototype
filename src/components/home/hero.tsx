...
import {
  CalendarIcon, Search, Luggage, Car, Map, Train, Plane, ShieldCheck, Eye, Sparkles, ChevronDown, Clock, Users, MapPin, Building2, Navigation
} from "lucide-react";
...
const serviceFields: Record<string, FormField[]> = {
  luggage: [
    ...
    { label: "Luggage", placeholder: "1-2 suitcases", type: "select", options: ["1-2 suitcases", "3-4 suitcases", "5+ suitcases"] },
  ],
  transfers: [
    { label: "Pick-up", placeholder: "Airport or Hotel", type: "text", icon: <Plane className="w-3.5 h-3.5 text-zinc-400" /> },
    { label: "Drop-off", placeholder: "Hotel or City", type: "text", icon: <Building2 className="w-3.5 h-3.5 text-zinc-400" /> },
    { label: "Date & Time", placeholder: "Select date", type: "date" },
    { label: "Passengers", placeholder: "1-3 guests", type: "select", options: ["1-3 guests", "4-6 guests", "7-10 guests", "10+ guests"] },
  ],
  taxi: [
    { label: "Pick-up", placeholder: "Your current location", type: "text", icon: <Navigation className="w-3.5 h-3.5 text-zinc-400" /> },
    { label: "Destination", placeholder: "Where to?", type: "text", icon: <MapPin className="w-3.5 h-3.5 text-zinc-400" /> },
    { label: "Date", placeholder: "Select date", type: "date" },
    { label: "Time", placeholder: "Now", type: "select", options: ["Now", "In 30 min", "In 1 hour", "Schedule later"] },
  ],
  ziyarat: [
    { label: "City", placeholder: "Makkah, Madinah, or Jeddah", type: "text", icon: <MapPin className="w-3.5 h-3.5 text-zinc-400" /> },
    { label: "Tour Type", placeholder: "Full day tour", type: "select", options: ["Full day tour", "Half day tour", "Private tour", "Group tour"] },
    { label: "Date", placeholder: "Select date", type: "date" },
    { label: "Group Size", placeholder: "1-4 people", type: "select", options: ["1-4 people", "5-10 people", "11-20 people", "20+ people"] },
  ],
};
...
export function Hero() {
  ...
  const tabs: ServiceTab[] = [
    ...
    { id: "transfers", label: "Transfers", icon: <Plane className="w-4 h-4" /> },
    { id: "taxi", label: "Taxi", icon: <Car className="w-4 h-4" /> },
    { id: "ziyarat", label: "Ziyarat", icon: <Map className="w-4 h-4" /> },
    { id: "trains", label: "Trains", icon: <Train className="w-4 h-4" />, comingSoon: true },
  ];
                      ...
                      <div className={cn("py-3 px-5 flex-1 min-w-0 rounded-lg hover:bg-zinc-50/80 transition-colors cursor-pointer", idx > 0 && "border-t border-zinc-100 lg:border-t-0")}>
                        ...
                        {field.type === "date" && (
                          <Popover>
                            <PopoverTrigger className="flex items-center justify-between w-full text-[14px] text-zinc-800 hover:text-zinc-900 px-1 -ml-1">
                              <span className={!date ? "text-zinc-400" : ""}>{date ? format(date, "PPP") : field.placeholder}</span>
                        ...
                        {field.type === "select" && (
                          <div className="relative">
                            <button
                              onClick={() => setSelectOpenIdx(selectOpenIdx === idx ? null : idx)}
                              className="flex items-center justify-between w-full text-[14px] text-zinc-800 hover:text-zinc-900 px-1 -ml-1"
                            >
                              <span className={!getSelectValue(idx) ? "text-zinc-400" : "text-zinc-800"}>
                                {getSelectValue(idx) || field.placeholder}
                              </span>
                              <ChevronDown className={cn("w-4 h-4 text-zinc-400 transition-transform", selectOpenIdx === idx && "rotate-180")} />
                            </button>
                            <AnimatePresence>
                                  ...
                                  {field.options.map((opt) => (
                                    <button
                                      ...
                                      className={cn(
                                        ...
                                        getSelectValue(idx) === opt ? "bg-amber-50 text-amber-700 font-medium" : "text-zinc-700"
                                      )}
                                    >
                                      {opt}
                                    </button>
